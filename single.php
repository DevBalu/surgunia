<?php 
	...
	if(!empty($con)){
		$result_images = mysqli_query($con, "SELECT i.* FROM images i WHERE i.folder_id = '$folder_id' " );
	}


	$images = '';
	while($resimg = $result_images->fetch_Object()){
		$images .= '
			<div class="grid-item item animate-box" data-animate-effect="fadeIn">
				<a href="' . $resimg->img_href . '" class="image-popup" title="' . $resimg->name . '">
					<div class="img-wrap">
						<img src="' . $resimg->img_href . '" alt="" class="img-responsive">
					</div>
					<div class="text-wrap">
...
<html>
	...
	<div class="back-to-home btn-circle">
		<a href="index.php"><span></span><em>Back</em></a>
	</div>
